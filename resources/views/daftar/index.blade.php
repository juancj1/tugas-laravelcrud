        ...
        <form method="POST" action="{{ route('logout') }}"> 
            ...
            <div class=" mb-10 ">
                ...
                <h1 class="text-[#151F3C] pt-36 text-7xl max-w-[600px] font-bold max-sm:text-3xl ">
                    List Orang yang Sudah Mendaftar
                </h1>
        ...
        <x-success-status class="mb-4" :status="session('message')" />
            <table class="table table-bordered w-full ">
                ...
                <tbody>
                    ...
                    <tr class="text-center">
                        <td class="border-2 border-slate-700">{{ $daftar->id }}</td>
                        <td class="border-2 border-slate-700">{{ $daftar->name }}</td>
                        <td class="border-2 border-slate-700">{{ $daftar->email }}</td>
                        <td class="border-2 border-slate-700 ">{{ $daftar->phone }}</td>
                        <td class="border-2 border-slate-700">{{ $daftar->gender }}</td>
                        <td class="border-2 border-slate-700 bg-green-700 text-white">
                            <a href="{{ url('/edit-daftar/'.$daftar->id) }}" class="">edit</a>
                        </td>
                        <td class="border-2 border-slate-700 bg-red-800 text-white">
                            
                            <form action="{{ url('delete-daftar/'.$daftar->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button class="">
                                    delete
